(()=>{
'use strict';
if(window.__ZUNO_LOADING_V2__)return;window.__ZUNO_LOADING_V2__=1;
...
function directSpinners(el){return [...el.children].filter(x=>x.matches?.('.zuno-loader,.loading-spinner')||(el.classList.contains('zp304-loading')&&x.matches?.('span:empty')))}
function existing(el){return directSpinners(el)[0]||null}
function hasLoadingAncestor(el){for(let p=el.parentElement;p;p=p.parentElement){if(p.dataset?.zunoLoadingEnhanced==='1'||p.matches?.('[data-zuno-loading],[aria-busy="true"],.loading,.zp-loading,.zn-loading,.zp304-loading,.zp325-loading,.zp326-loading'))return true}return false}
function clear(el){if(!el||el.dataset?.zunoLoadingEnhanced!=='1')return;for(const s of directSpinners(el)){if(s.dataset.zunoInjected==='1')s.remove();else s.classList.remove('zuno-loader','zuno-loader--sm','zuno-loader--lg')}el.classList.remove('zuno-loading-state','zuno-loading-inline');delete el.dataset.zunoLoadingEnhanced}
function enhance(el){if(!isLoading(el)){clear(el);return false}if(hasLoadingAncestor(el)){clear(el);return false}const inline=el.matches('button,a,label,small,span,[role="button"]')&&!el.matches('.loading,.zp304-loading,.zp325-loading,.zp326-loading');let s=existing(el);if(s){s.classList.add('zuno-loader');s.classList.toggle('zuno-loader--sm',inline)}else{s=spinner(inline?'sm':'md');s.dataset.zunoInjected='1';el.prepend(s)}for(const extra of directSpinners(el).slice(1)){if(extra.dataset.zunoInjected==='1')extra.remove();else extra.style.display='none'}el.classList.toggle('zuno-loading-state',!inline);el.classList.toggle('zuno-loading-inline',inline);el.dataset.zunoLoadingEnhanced='1';if(!el.hasAttribute('role')&&!inline)el.setAttribute('role','status');if(!el.hasAttribute('aria-live')&&!inline)el.setAttribute('aria-live','polite');return true}
...
function scan(root=document){const list=[...candidates(root)];list.sort((a,b)=>{let d=0,p=a;while(p=p.parentElement)d++;let e=0,q=b;while(q=q.parentElement)e++;return d-e});list.forEach(enhance)}
