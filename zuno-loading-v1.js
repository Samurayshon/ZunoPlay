...
function enhance(el){if(!isLoading(el)){if(el?.dataset?.zunoLoadingEnhanced==='1'){el.classList.remove('zuno-loading-state','zuno-loading-inline');delete el.dataset.zunoLoadingEnhanced}return false}const inline=el.matches('button,a,label,small,span,[role="button"]')&&!el.matches('.loading,.zp304-loading,.zp325-loading,.zp326-loading');let s=existing(el);if(s)s.classList.add('zuno-loader',inline?'zuno-loader--sm':'');else{ s=spinner(inline?'sm':'md');el.prepend(s)}el.classList.toggle('zuno-loading-state',!inline);el.classList.toggle('zuno-loading-inline',inline);el.dataset.zunoLoadingEnhanced='1';if(!el.hasAttribute('role')&&!inline)el.setAttribute('role','status');if(!el.hasAttribute('aria-live')&&!inline)el.setAttribute('aria-live','polite');return true}
function candidates(root=document){const set=new Set();if(root?.nodeType===1){if(root.matches?.(KNOWN))set.add(root);root.querySelectorAll?.(KNOWN).forEach(x=>set.add(x));root.querySelectorAll?.('div,p,span,small,strong,button,label').forEach(x=>{const t=text(x);if(t&&t.length<=180&&WORDS.test(t))set.add(x)})}else document.querySelectorAll(KNOWN+',div,p,span,small,strong,button,label').forEach(x=>set.add(x));return set}
...
function stop(el){if(!el)return;el.removeAttribute('data-zuno-loading');el.removeAttribute('aria-busy');el.querySelector(':scope > .zuno-loader')?.remove();el.classList.remove('zuno-loading-state','zuno-loading-inline');delete el.dataset.zunoLoadingEnhanced}
