(()=>{
  ...
  function mountHeader(){
    ...
    hideLegacyHeaders();
    document.body.classList.add('zuno-global-header-mounted');
  }
  function mountBottom(){
    ...
    document.body.classList.add('zuno-global-nav-mounted');
  }
  function ensureSearch(){
    if(document.querySelector('.zuno-search-layer'))return;
    const el=document.createElement('div');
    el.className='zuno-search-layer';
    ...
    const input=el.querySelector('.zuno-search-input'),results=el.querySelector('.zuno-search-results');
    let timer=0,seq=0;
    const esc=s=>{const d=document.createElement('div');d.textContent=s==null?'':String(s);return d.innerHTML};
    async function run(q){
      const my=++seq;
      if(q.length<2){results.innerHTML='<div class="zuno-search-empty">Digite pelo menos 2 caracteres para buscar.</div>';return}
      ...
      try{
        ...
        const[p,r]=await Promise.all([sb.from('profiles').select('id,username,level').ilike('username',`%${q}%`).limit(8),sb.from('rooms').select('id,name').ilike('name',`%${q}%`).limit(8)]);
        if(my!==seq)return;
    ...
    el.querySelector('.zuno-search-close').onclick=()=>el.classList.remove('open');
