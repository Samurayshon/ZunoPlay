(() => {
  ...
  function loadScript(id,file,errorText){if(document.getElementById(id))return;const s=document.createElement('script');s.id=id;s.src=new URL(versioned(file),location.href).href;s.async=true;s.onerror=()=>console.error(errorText);document.head.appendChild(s)}
  ...
  function loadOfficialAvatars(){loadScript('zunoplay-official-avatars','./official-avatars.js','ZunoPlay: não foi possível carregar o sistema de avatares')}
  function loadAvatarCustomizer(){if(page==='avatar.html')loadScript('zunoplay-avatar-customizer','./avatar-customizer.js','ZunoPlay: não foi possível carregar o sistema de personalização')}
  function loadOfficialHome(){if(page!=='index.html')return;loadStyle('zunoplay-home-style','./home-v29.css');loadScript('zunoplay-home-script','./home-v29.js','ZunoPlay: não foi possível carregar a Home oficial')}
  ...
  function bootstrapRealtime(){if(patchSupabaseFactory()){loadRealtimeCore();loadOfficialAvatars();loadAvatarCustomizer();return}if(document.getElementById('zunoplay-supabase-sdk'))return;const sdk=document.createElement('script');sdk.id='zunoplay-supabase-sdk';sdk.src='https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2';sdk.async=true;sdk.onload=()=>{if(patchSupabaseFactory()){loadRealtimeCore();loadOfficialAvatars();loadAvatarCustomizer()}};sdk.onerror=()=>console.error('ZunoPlay: não foi possível carregar Supabase');document.head.appendChild(sdk)}

  bootstrapRealtime();loadRoomVoice();loadRoomSessionGuard();loadOfficialHome();
  const brandReady=()=>{mountOfficialLogo();setTimeout(mountOfficialLogo,250)};if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',brandReady,{once:true});else brandReady();
  ...
  function schedule(){if(scheduled)return;scheduled=true;requestAnimationFrame(()=>{scheduled=false;mount()})}function ready(){mount();schedule()}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',ready,{once:true});else ready();const observer=new MutationObserver(schedule);observer.observe(document.documentElement,{childList:true,subtree:true});
})();