(() => {
  ...
  function patchSupabaseFactory(){
    ...
    window.supabase.createClient=function(url,key,options){const sameProject=url===SUPABASE_URL&&key===SUPABASE_KEY;const defaultOptions=!options||Object.keys(options).length===0;if(sameProject&&defaultOptions)return window.ZunoSupabaseClient;return originalCreateClient(url,key,options)};
    window.__ZUNOPLAY_SUPABASE_FACTORY_PATCHED__=true;return true;
  }
  ...
  loadDesignSystem();loadUnifiedIdentity();loadStage3Navigation();loadSocialSystem();bootstrapRealtime();loadRoomVoice();loadRoomSessionGuard();loadOfficialHome();
