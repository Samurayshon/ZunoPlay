(()=>{
'use strict';
if(window.__ZUNO_PULSO_INTERACTIONS_V326__)return;window.__ZUNO_PULSO_INTERACTIONS_V326__=1;
...
function closeLikes(){const m=$('#zp326Likes');if(m)m.hidden=true}
async function openLikes(postId){
  if(!postId||!await ready())return;
  ensureLikes();
  const modal=$('#zp326Likes'),list=$('#zp326LikesList');
  modal.hidden=false;
  list.innerHTML='<div class="zp326-loading">Carregando curtidas...</div>';
  try{
    const{data:likes,error}=await client.from('moments_likes').select('user_id,created_at').eq('post_id',postId).order('created_at',{ascending:false});
    ...
    const{data:profiles,error:pe}=await client.from('profiles').select('id,username,nickname,avatar_url,level').in('id',ids);
    if(pe)throw pe;
    ...
    console.error('[Pulso likes v326]',e);
    list.innerHTML='<div class="zp326-empty"><b>Não foi possível carregar as curtidas</b><span>Tente novamente em instantes.</span></div>';
  }
}
