(()=>{
  ...
  const catMeta={
    Base:{label:'Avatar',icon:'◉',order:1},Roupas:{label:'Roupas',icon:'♢',order:2},Acessórios:{label:'Acessórios',icon:'✧',order:3},
    Cabelo:{label:'Cabelos',icon:'✦',order:4},Rosto:{label:'Rosto',icon:'☺',order:5},Calçados:{label:'Calçados',icon:'▰',order:6},
    Efeitos:{label:'Auras',icon:'✦',order:7},Mascote:{label:'Mascotes',icon:'◆',order:8}
  };
  ...
  function currentCat(){const t=(q('#panelTitle')?.textContent||'Base').trim();return t==='Tipo de corpo'?'Base':t}
  ...
  function syncSubtabs(){const cat=currentCat();qa('.studio-subtab').forEach(b=>b.classList.toggle('on',b.dataset.cat===cat));if(cat==='Base'&&q('#panelTitle'))q('#panelTitle').textContent='Tipo de corpo'}
  ...
  function updatePreviewRail(){
    ...
    const top=q('.zuno-global-action.is-profile img');if(top)mountPreview(top,profile);
  }
  ...
  function decorateOptionThumbs(){
    ...
    qa('#options .option').forEach((opt,i)=>{
      ...
      if(cat==='Base'){
        let img=thumb.querySelector('img');if(!img){thumb.textContent='';img=document.createElement('img');thumb.appendChild(img)}
  ...
  function makeSubtabs(){
    ...
    subcats.forEach(([label,cat])=>{const b=document.createElement('button');b.type='button';b.className='studio-subtab';b.dataset.cat=cat;b.textContent=label;b.onclick=()=>clickRealCategory(cat);nav.appendChild(b)});return nav;
  }
  function randomize(){
  ...
  function buildOfficialCard(title,rarity,cfg,opts={}){
    const b=document.createElement('button');b.type='button';b.className='studio-official-card'+(opts.active?' active':'');b._studioCfg=clone(cfg);
    ...
    mountPreview(b.querySelector('img'),b._studioCfg);
  ...
  function makeOfficials(){
    ...
    const list=left.querySelector('.studio-official-list'),r=window.ZunoAvatarRenderer,base=clone(r?.official?.masculino||getState());base.mode='Corpo inteiro';base.zoom=1;
    const variants=[
  ...
  function refreshOfficials(){qa('.studio-official-card').forEach(b=>{if(b._studioCfg)mountPreview(b.querySelector('img'),b._studioCfg)})}

  function ensureTopAvatar(){
    const profile=q('.zuno-global-action.is-profile');if(!profile)return;
    if(!profile.querySelector('img')){profile.textContent='';const img=document.createElement('img');img.alt='Seu avatar';profile.appendChild(img)}
  }
  function scheduleRefresh(){clearTimeout(refreshTimer);refreshTimer=setTimeout(()=>{decorateTabs();decorateOptionThumbs();syncSubtabs();updatePreviewRail();refreshOfficials()},45)}
  ...
  function mount(){
    ...
    editor.append(rail,stageCol,makePreviewRail());app.insertBefore(editor,panel);app.insertBefore(makeSubtabs(),panel);
    ...
    ensureTopAvatar();decorateTabs();decorateOptionThumbs();syncSubtabs();updatePreviewRail();refreshOfficials();

    new MutationObserver(scheduleRefresh).observe(tabs,{childList:true,subtree:true});new MutationObserver(scheduleRefresh).observe(q('#options'),{childList:true,subtree:true});new MutationObserver(scheduleRefresh).observe(q('#avatarPreview'),{attributes:true,attributeFilter:['src','data-zuno-model']});
