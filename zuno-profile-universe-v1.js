(()=>{
if(window.__ZUNO_PROFILE_UNIVERSE_V1__)return;window.__ZUNO_PROFILE_UNIVERSE_V1__=true;
const REF='rliymfbbhqoejgfvsbuu',BASE='https://'+REF+'.supabase.co',KEY='sb_publishable_E4go4X7yZ6d-aXnKAT-fWw_Y8uHIJT0';
const qs=new URLSearchParams(location.search),target=qs.get('user');let timer=null;
...
get('user_presence?select=status,custom_status,page,last_seen_at&user_id=eq.'+encodeURIComponent(id)+'&limit=1'),
get('friendships?select=user_id,friend_id,created_at&or=(user_id.eq.'+id+',friend_id.eq.'+id+')'),
get('moments_posts?select=id,content,media_type,created_at&user_id=eq.'+id+'&order=created_at.desc&limit=3'),
...
const communitiesHtml=memberships.length?memberships.map(m=>'<span class="zpu-chip">'+esc(m.communities?.name||'Comunidade')+'</span>').join(''):'<span class="zpu-muted">Nenhuma comunidade ainda.</span>';
...
'<section class="zpu-card zpu-section"><div class="zpu-head"><div class="zpu-title">Jogos</div></div><div class="zpu-game"><div class="zpu-game-icon">🎮</div><div class="zpu-game-copy"><b>ZunoPlay Games</b><span>Nível '+(gp?.game_level||1)+' · '+(gp?.total_games||0)+' partidas · '+(gp?.total_wins||0)+' vitórias · recorde '+(gp?.best_score||0)+'</span></div></div></section>'+
'<section class="zpu-card zpu-section"><div class="zpu-head"><div class="zpu-title">Comunidades</div><button class="zpu-link" data-go="comunidades.html">Ver todas</button></div><div class="zpu-community-list">'+communitiesHtml+'</div></section>'+
'<section class="zpu-card zpu-section"><div class="zpu-head"><div class="zpu-title">Amigos</div><button class="zpu-link" data-go="amigos.html">Ver todos</button></div><div class="zpu-friend-list">'+friendsHtml+'</div></section>'+
...
}catch(e){console.error('profile-universe',e);root.removeAttribute('aria-busy');root.innerHTML='<div class="zpu-card zp-error">Não foi possível carregar o perfil social.<br><button class="zp-retry" onclick="location.reload()">Tentar novamente</button></div>'}}
