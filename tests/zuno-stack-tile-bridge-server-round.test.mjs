...
const engine={
  active:true,
  tiles:Array.from({length:90},(_,i)=>({id:`t${i}`,type:`p${i%10}`,x:i%6,y:Math.floor(i/6)%6,layer:0,removed:false})),
  tray:[],score:0,matches:0
};
let capture=null,applyCalls=0,reconcileCalls=0;
const authority={
  getState(){return {revision:2,roomId}},
...
const sb={from(name){assert.equal(name,'zuno_stack_match_state');return {
  ...
  async maybeSingle(){return {data:{revision:2,state:{engine}},error:null}}
}}};
...
assert.equal(applyCalls,1,'tile bridge did not reach authority.applyTile');
assert.ok(reconcileCalls>=1,'active server round was not reconciled before tile application');
console.log('Zuno Stack tile bridge server-round path OK');
