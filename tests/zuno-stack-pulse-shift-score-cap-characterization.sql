...
-- room_members has a session-aware trigger, so each membership is inserted with the same
-- JWT subject that owns that room, matching the already validated RPC fixture pattern.
