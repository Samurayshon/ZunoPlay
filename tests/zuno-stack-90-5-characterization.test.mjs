...
assert.match(tile,/jsonb_array_length\(v_tiles\)<>90/,'tile RPC must require 90 tiles');
assert.match(tile,/jsonb_array_length\(v_relay\)<>3/,'tile RPC must require 3 relay slots');
assert.match(tile,/jsonb_array_length\(v_tray\)>=7/,'tile RPC must reject picks when tray is already full');
...
assert.match(tile,/v_score:=least\(25000,v_score\+25\)/,'normal tile pick must add 25 points');
assert.match(tile,/order by e\.ord desc limit 3/,'trio must remove the three most-recent matching tray occurrences');
assert.match(tile,/v_now_ms-v_last_match<=4000/,'combo window must remain 4000ms inclusive');
assert.match(tile,/v_matches:=v_matches\+1/,'a resolved trio must increment matches exactly through the authoritative path');
assert.match(tile,/v_energy:=least\(5,v_energy\+1\)/,'standard trio energy gain/cap changed');
assert.match(tile,/if v_matches%5=0 then/,'automatic Pulse cycle must trigger each fifth match');
assert.match(tile,/case when v_double then 620 else 310 end/,'standard trio base/double scoring changed');
assert.match(tile,/\(\(v_combo-1\)\*55\)/,'combo score increment changed');
...
assert.match(tile,/if v_tiles_left=0 then v_active:=false; v_kind:='win'; elsif jsonb_array_length\(v_tray\)>=7 and not v_has_match then v_active:=false; v_kind:='finish'/,'tile completion policy changed');
...
assert.match(relay,/v_now_ms-v_last_match<=4000/,'relay-take combo window must remain aligned at 4000ms');
assert.match(relay,/v_matches:=v_matches\+1/,'relay-take trio must increment matches');
assert.match(relay,/v_energy:=least\(5,v_energy\+1\)/,'relay-take trio energy gain/cap changed');
assert.match(relay,/if v_matches%5=0 then/,'relay-take automatic Pulse cycle changed');
