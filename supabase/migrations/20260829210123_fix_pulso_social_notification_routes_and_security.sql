...
begin
  ...
  if target_user is not null and actor is not null and target_user <> actor then
    dkey := kind || ':' || actor || ':' || coalesce(rel::text, target_user::text) || ':' ||
      coalesce(case when tg_table_name = 'moments_comments' then new.id::text else null end, '');

    insert into public.notifications(
      user_id,type,title,message,related_user_id,related_id,category,priority,action_url,dedupe_key,metadata
    ) values (
      target_user,
      kind,
      'Pulso',
