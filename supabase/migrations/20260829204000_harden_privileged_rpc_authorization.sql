-- ZunoPlay QA/Security: harden privileged RPC authorization boundaries.
-- 1) Prevent authenticated users from probing friendship state between two third parties.
-- 2) Prevent authenticated users from probing moderator status for another user.
-- 3) Prevent room admins from banning the room owner through ban_room_member().
