...
begin
  if tg_table_name='moments_likes' then
    ...
    actor:=new.user_id;
    select c.user_id,c.post_id into target_user,rel from public.moments_comments c where c.id=new.comment_id;
    kind:='pulso_comment_like'; msg:='curtiu seu comentário'; event_suffix:=new.comment_id::text;
  ...
  if target_user is not null and actor is not null and target_user<>actor then
    ...
    perform private.zuno_emit_notification(
      ...
      dkey,jsonb_build_object('surface','pulso','event_type',kind,'actor_username',coalesce(actor_name,'Alguém'),'comment_id',case when tg_table_name='moments_comment_likes' then new.comment_id else null end),
      null,true
    );
  end if;
  return new;
...
drop trigger if exists trg_pulso_comment_like_notification on public.moments_comment_likes;
create trigger trg_pulso_comment_like_notification
after insert on public.moments_comment_likes
for each row execute function public.pulso_emit_notification();
