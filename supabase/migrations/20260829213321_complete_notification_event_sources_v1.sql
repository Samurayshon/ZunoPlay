...
declare v_owner uuid; v_actor text; v_room text;
begin
...
declare v_actor text; v_room text; v_type text; v_title text; v_state text;
begin
  if new.status is not distinct from old.status then return null; end if;
  if new.status='approved' then v_type:='room_seat_approved'; v_state:='accepted';
  elsif new.status='rejected' then v_type:='room_seat_rejected'; v_state:='rejected';
...
declare v_actor text; v_room text; v_title text;
begin
...
begin
  if new.revoked_at is not null then v_state:='cancelled';
  elsif new.accepted_at is not null then v_state:='accepted';
  elsif new.expires_at<=now() then v_state:='expired';
...
declare v_actor text; v_type text; v_title text; v_target uuid; v_state text;
begin
  if new.status is not distinct from old.status then return null; end if;
  ...
  elsif new.status='completed' then v_type:='game_challenge_completed'; v_title:='Desafio concluído'; v_target:=new.challenger_id; v_state:='completed';
  else return null; end if;
