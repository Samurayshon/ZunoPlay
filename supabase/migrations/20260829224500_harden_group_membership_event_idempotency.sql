...
begin
  ...
  -- Idempotent retry: an existing member must not emit a new system event.
  if exists(
  ...
  -- A concurrent request may have inserted the same membership first.
  if not found then
    return;
  end if;
...
begin
  ...
  -- Idempotent retry: a member already absent must not emit a fake event.
  ...
  -- If a concurrent request removed it first, do not forge a removal event.
  if not found then
    return;
  end if;
