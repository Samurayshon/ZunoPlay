-- ZunoPlay social -> messages bridge.
-- When a pending friend request becomes accepted, create the friendship,
-- guarantee the direct conversation, unarchive it for both users, and insert
-- an idempotent system message so the original sender sees the conversation
-- immediately in Messages and can start chatting.
