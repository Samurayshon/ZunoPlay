...
begin
  ...
  -- Once a round is active, the Relay array is server-owned regardless of the
  -- client-supplied state kind. This prevents bypassing the guard with `mutation`
  -- or another legacy kind.
  if v_old_active
...
begin
  ...
  if (p_mode='send' and (p_index<0 or p_index>6))
     or (p_mode='take' and (p_index<0 or p_index>2)) then
  ...
  -- Re-check idempotency after the state-row lock to close concurrent retry races.
  select * into v_existing
  from public.zuno_stack_game_events e
  where e.room_id=p_room_id and e.action_id=p_action_id;
  if found then
  ...
  if exists(
    ...
    where e.room_id=p_room_id and e.actor_id=v_user
      ...
      and e.created_at > clock_timestamp() - interval '60 milliseconds'
  ) then
  ...
  if p_mode='send' then
    if p_index >= jsonb_array_length(v_tray) then
      raise exception 'stack_tray_index_invalid' using errcode='22023';
    end if;
    select e.val into v_type
    from jsonb_array_elements_text(v_tray) with ordinality as e(val,ord)
    where e.ord=p_index+1;
    ...
    if v_slot is null then
      raise exception 'stack_relay_full' using errcode='22023';
    end if;
    ...
    v_score := least(25000,v_score+20);
    v_kind := 'server_relay_send';
  else
    ...
    if v_type is null or length(v_type)=0 then
      raise exception 'stack_relay_empty' using errcode='22023';
    end if;
    ...
    -- Core semantics add the Relay receive bonus after resolving a possible trio.
    ...
    if jsonb_array_length(v_tray)>=7 and not v_has_match then
      v_active := false;
      v_kind := 'finish';
    else
      v_kind := 'server_relay_take';
    end if;
  end if;
  ...
  values(p_room_id,p_action_id,v_event_type,jsonb_build_object(
    'index',p_index,
    'type',v_type,
    'expected_revision',p_expected_revision,
    'applied_revision',v_new_revision,
    'matched',p_mode='take' and v_type_count>=3,
    'outcome',v_kind,
    'relay_slot',case when p_mode='send' then v_slot else null end
  ),v_user);
...
create or replace function public.zuno_stack_relay_send(
  p_room_id uuid,
  p_expected_revision bigint,
  p_action_id text,
  p_tray_index integer
)
returns public.zuno_stack_match_state
language sql
security invoker
set search_path = ''
as $function$
  select zuno_private.zuno_stack_apply_relay_internal(p_room_id,p_expected_revision,p_action_id,'send',p_tray_index);
$function$;

create or replace function public.zuno_stack_relay_take(
  p_room_id uuid,
  p_expected_revision bigint,
  p_action_id text,
  p_relay_index integer
)
returns public.zuno_stack_match_state
language sql
security invoker
set search_path = ''
as $function$
  select zuno_private.zuno_stack_apply_relay_internal(p_room_id,p_expected_revision,p_action_id,'take',p_relay_index);
$function$;
