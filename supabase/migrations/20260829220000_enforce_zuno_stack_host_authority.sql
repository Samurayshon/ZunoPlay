...
begin
  ...
  -- Host metadata may only be claimed/refreshed by the caller when the lease
  -- is free, theirs, or expired; release is only allowed by the current host.
...
begin
  ...
  -- A human client should never need to request actions faster than this. The
  -- unique action id remains the idempotency boundary for retries.
  if exists(
