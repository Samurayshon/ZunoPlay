-- Prevent repeated end_voice_room calls from generating duplicate audit rows
-- or re-running cleanup after the room is already ended.
...
begin
  ...
  -- The room was already ended. Treat the operation as a successful no-op
  -- so retries are safe and do not create duplicate side effects.
