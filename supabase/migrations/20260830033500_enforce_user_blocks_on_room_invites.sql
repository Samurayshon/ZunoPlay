-- Enforce user blocking consistently across direct room invitations.
-- A blocked pair must not be able to create/receive a targeted invite notification,
-- and an invite token issued by a blocked inviter must not authorize private-room entry.
