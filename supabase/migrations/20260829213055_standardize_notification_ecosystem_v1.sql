...
do $$ begin
  if not exists (select 1 from pg_constraint where conname='notifications_action_state_chk') then
    alter table public.notifications add constraint notifications_action_state_chk check (action_state in ('none','pending','accepted','rejected','cancelled','expired','completed'));
  end if;
end $$;
create index if not exists notifications_user_state_idx on public.notifications(user_id,action_state,created_at desc);
...
begin return private.zuno_emit_notification(p_user_id,p_type,p_title,p_message,p_actor_id,p_entity_id,p_category,p_priority,p_action_url,p_dedupe_key,p_metadata,p_expires_at,false); end $$;
...
create or replace function public.zuno_mark_notifications_read(p_scope text default 'all') returns integer language plpgsql set search_path='' as $$ declare v_user uuid:=auth.uid(); v_count int; begin if v_user is null then raise exception 'authentication_required' using errcode='42501'; end if; if p_scope not in ('all','friend_request','message','social','rooms','games','rewards','system') then raise exception 'invalid_notification_scope' using errcode='22023'; end if; update public.notifications n set read_at=coalesce(n.read_at,now()),seen_at=coalesce(n.seen_at,now()) where n.user_id=v_user and n.read_at is null and (p_scope='all' or n.category=p_scope); get diagnostics v_count=row_count; return v_count; end $$;
create or replace function public.zuno_mark_notifications_seen() returns integer language plpgsql set search_path='' as $$ declare v_user uuid:=auth.uid(); v_count int; begin if v_user is null then raise exception 'authentication_required' using errcode='42501'; end if; update public.notifications n set seen_at=coalesce(n.seen_at,now()) where n.user_id=v_user and n.seen_at is null; get diagnostics v_count=row_count; return v_count; end $$;
revoke all on function public.zuno_mark_notifications_read(text) from public,anon; grant execute on function public.zuno_mark_notifications_read(text) to authenticated;
revoke all on function public.zuno_mark_notifications_seen() from public,anon; grant execute on function public.zuno_mark_notifications_seen() to authenticated;
