...
begin
  if tg_table_name='moments_likes' then
    ...
    kind:='pulso_like'; msg:='curtiu seu comentário'; event_suffix:=new.comment_id::text; reaction_target:='comment';
  ...
  if target_user is not null and actor is not null and target_user<>actor then
    ...
    perform private.zuno_emit_notification(
      ...
      dkey,
      jsonb_strip_nulls(jsonb_build_object('surface','pulso','event_type',case when reaction_target='comment' then 'pulso_comment_like' else kind end,'actor_username',coalesce(actor_name,'Alguém'),'reaction_target',reaction_target,'comment_id',case when reaction_target='comment' then new.comment_id else null end)),
      null,true
    );
  end if;
  return new;
