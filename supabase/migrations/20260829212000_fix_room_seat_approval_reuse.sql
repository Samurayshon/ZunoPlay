-- Etapa 19 / QA security hardening
-- Approved seat requests are resolved by resolve_room_seat_request(), which already
-- places the member on stage. Allowing take_room_seat() to reuse any historical
-- approved request turns that approval into a persistent capability after the
-- member leaves or is demoted. Only open rooms or moderators may self-take a seat.
...
begin
  ...
  -- For request/invite-only rooms the moderator approval path is
  -- resolve_room_seat_request(), which seats the member atomically.
  -- A previously approved request must not be reusable after leaving/demotion.
