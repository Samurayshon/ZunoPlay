...
alter policy friend_request_events_select_participants on public.friend_request_events using (((select auth.uid()) = sender_id) or ((select auth.uid()) = receiver_id));
alter policy friendship_events_select_participants on public.friendship_events using (((select auth.uid()) = user_id) or ((select auth.uid()) = friend_id));
