...
begin
  ...
  if p_power='fluxo' then
    ...
    with active as (
      select e.ord::integer ord,e.tile from jsonb_array_elements(v_tiles) with ordinality e(tile,ord)
      where not coalesce((e.tile->>'removed')::boolean,false)
        and not exists(select 1 from jsonb_array_elements(v_tiles) h(tile) where not coalesce((h.tile->>'removed')::boolean,false) and (h.tile->>'x')::integer=(e.tile->>'x')::integer and (h.tile->>'y')::integer=(e.tile->>'y')::integer and (h.tile->>'layer')::integer>(e.tile->>'layer')::integer)
    ), grouped as (select tile->>'type' typ,count(*) n,min(ord) first_ord from active group by tile->>'type')
    ...
    with active as (
      select e.ord::integer ord,e.tile from jsonb_array_elements(v_tiles) with ordinality e(tile,ord)
      where not coalesce((e.tile->>'removed')::boolean,false) and e.tile->>'type'=v_active_type
        and not exists(select 1 from jsonb_array_elements(v_tiles) h(tile) where not coalesce((h.tile->>'removed')::boolean,false) and (h.tile->>'x')::integer=(e.tile->>'x')::integer and (h.tile->>'y')::integer=(e.tile->>'y')::integer and (h.tile->>'layer')::integer>(e.tile->>'layer')::integer)
      order by e.ord limit 3
    ), chosen as (select array_agg(ord) ords from active)
    ...
    if p_power='elo' then
      ...
      select e.tile,e.ord::integer into v_target,v_target_ord from jsonb_array_elements(v_tiles) with ordinality e(tile,ord)
      where not coalesce((e.tile->>'removed')::boolean,false)
        and exists(select 1 from jsonb_array_elements(v_tiles) h(tile) where not coalesce((h.tile->>'removed')::boolean,false) and (h.tile->>'x')::integer=(e.tile->>'x')::integer and (h.tile->>'y')::integer=(e.tile->>'y')::integer and (h.tile->>'layer')::integer>(e.tile->>'layer')::integer)
      order by (select count(*) from jsonb_array_elements_text(v_tray) q(val) where q.val=e.tile->>'type') desc,(e.tile->>'layer')::integer desc,e.ord limit 1;
    else
      select e.tile,e.ord::integer into v_target,v_target_ord from jsonb_array_elements(v_tiles) with ordinality e(tile,ord)
      where not coalesce((e.tile->>'removed')::boolean,false)
        and not exists(select 1 from jsonb_array_elements(v_tiles) h(tile) where not coalesce((h.tile->>'removed')::boolean,false) and (h.tile->>'x')::integer=(e.tile->>'x')::integer and (h.tile->>'y')::integer=(e.tile->>'y')::integer and (h.tile->>'layer')::integer>(e.tile->>'layer')::integer)
        and exists(select 1 from jsonb_array_elements_text(v_tray) q(val) where q.val=e.tile->>'type')
      order by (select count(*) from jsonb_array_elements_text(v_tray) q(val) where q.val=e.tile->>'type') desc,e.ord limit 1;
    end if;
    if v_target is null then raise exception 'stack_power_no_target' using errcode='22023'; end if;
