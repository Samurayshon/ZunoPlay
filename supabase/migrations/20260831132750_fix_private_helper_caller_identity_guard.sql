...
begin
  if v_uid is not null and p_user_id is distinct from v_uid then return false; end if;
...
begin
  if v_uid is not null and p_user_id is distinct from v_uid then return false; end if;
  return exists(select 1 from public.rooms r where r.id=p_room_id and (r.visibility='public' or r.owner_id=p_user_id or exists(select 1 from public.room_members rm where rm.room_id=p_room_id and rm.user_id=p_user_id) or (r.visibility='friends' and exists(select 1 from public.friendships f where (f.user_id=r.owner_id and f.friend_id=p_user_id) or (f.friend_id=r.owner_id and f.user_id=p_user_id)))));
end $function$;
