const CACHE_NAME = "zunoplay-v184";
...
const STATIC_FILES = [
  ...
  "./zuno-design-system.css","./zuno-unified.css","./zuno-unified.js","./zuno-social.css","./zuno-social.js","./zuno-current-shell-v173.css","./zuno-home-shell-v180.css","./zuno-global-shell-v184.css","./zuno-global-shell-v184.js","./zuno-current.js","./zuno-current-base.css","./zuno-current-stage.css","./zuno-current-home.css","./zuno-current-home.js","./zuno-current-home-mobile.css","./zuno-current-home-stats.css","./zuno-current-interactions.css",
...
function retryAfterMs(response){const raw=response?.headers?.get?.('retry-after');if(!raw)return 60000;const seconds=Number(raw);if(Number.isFinite(seconds))return Math.max(1000,seconds*1000);const date=Date.parse(raw);return Number.isFinite(date)?Math.max(1000,date-Date.now()):60000}
...
function isCurrentUiAsset(url){
  ...
  return path.includes('/zuno-current')||
    path.endsWith('/zuno-global-shell-v184.css')||
    path.endsWith('/zuno-global-shell-v184.js')||
...
self.addEventListener('fetch',event=>{
  ...
  if(request.mode==='navigate'){
    event.respondWith((async()=>{try{const fresh=await dedupedFetch(request,{cache:'no-store'},4000);if(fresh?.ok){event.waitUntil(cachePut(request,fresh));return fresh}}catch(_){}const cached=await cacheMatch(request,true);if(cached)return cached;const fallback=await cacheMatch('./index.html',true);if(fallback)return fallback;return new Response('ZunoPlay temporariamente indisponível. Tente novamente.',{status:503,headers:{'Content-Type':'text/plain; charset=utf-8'}})})());return;
  }
  if(isCurrentUiAsset(url)){
    event.respondWith((async()=>{try{const fresh=await dedupedFetch(request,{cache:'no-store'},4000);if(fresh?.ok){event.waitUntil(cachePut(request,fresh));return fresh}}catch(_){}const exact=await cacheMatch(request,false);if(exact)return exact;const sameGeneration=await cacheMatch(request,true);if(sameGeneration)return sameGeneration;return new Response('Recurso de interface indisponível.',{status:503,headers:{'Content-Type':'text/plain; charset=utf-8'}})})());return;
  }
  event.respondWith((async()=>{const cached=await cacheMatch(request,false);if(cached)return cached;try{const fresh=await dedupedFetch(request,{cache:'no-cache'},5000);if(fresh?.ok)event.waitUntil(cachePut(request,fresh));return fresh}catch(_){const sameGeneration=await cacheMatch(request,true);if(sameGeneration)return sameGeneration;return new Response('Recurso indisponível offline.',{status:503,headers:{'Content-Type':'text/plain; charset=utf-8'}})}})());
});
