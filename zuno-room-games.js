...
function installPlayIntercept(){document.addEventListener('click',e=>{const btn=e.target.closest?.('[data-action="play"]');if(!btn)return;e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();openStack()},{capture:true})}
function init(){if(!document.body.classList.contains('zuno-page-sala'))return;installPlayIntercept()}
