...
function validateVercelEnvironment(env = process.env) {
  ...
  if (!url || !key) {
    throw new Error('Preview/Staging requires explicit ZUNO_SUPABASE_URL and ZUNO_SUPABASE_PUBLISHABLE_KEY.');
  }
  if (url.includes(PROD_REF)) {
    throw new Error('Preview/Staging must not use the production Supabase project.');
  }
  if (!/^https:\/\/[a-z0-9-]+\.supabase\.co\/?$/i.test(url)) {
    throw new Error('ZUNO_SUPABASE_URL is not a valid Supabase project URL.');
  }
  if (!isPublishableKey(key)) {
    throw new Error('Preview/Staging requires a publishable/anon-compatible client key, never a service-role/secret key.');
  }

  const refs = findProductionBackendRefs();
  if (refs.length) {
    throw new Error(`Frontend is still coupled to production Supabase in: ${refs.join(', ')}`);
  }
}
...
function selfTest() {
  ...
  expectFailure(
    'Vercel production authority is rejected',
    { VERCEL_ENV: 'production', ZUNO_SUPABASE_URL: 'https://example.supabase.co', ZUNO_SUPABASE_PUBLISHABLE_KEY: 'sb_publishable_test' },
    /production is forbidden/i,
  );
  expectFailure(
    'missing staging backend fails closed',
    { VERCEL_ENV: 'preview' },
    /requires explicit/i,
  );
  expectFailure(
    'production Supabase is rejected for preview',
    { VERCEL_ENV: 'preview', ZUNO_SUPABASE_URL: PROD_URL, ZUNO_SUPABASE_PUBLISHABLE_KEY: 'sb_publishable_test' },
    /must not use the production/i,
  );
}
...
if (process.env.VERCEL !== '1') {
  throw new Error('This guard is intended for Vercel builds. Set VERCEL=1 only in the real Vercel build environment.');
}
