(()=>{
  ...
  function installV167Refresh(){if(!('serviceWorker' in navigator))return;const key='zuno-sw-v167-reloaded';navigator.serviceWorker.addEventListener('controllerchange',()=>{if(sessionStorage.getItem(key)==='1')return;sessionStorage.setItem(key,'1');location.reload()})}
  function loadCurrentModules(){
    injectStyle('zunoplay-current-stage-style','./zuno-current-stage.css');
    if(page==='index.html'){
      injectStyle('zunoplay-current-home-style','./zuno-current-home.css');injectScript('zunoplay-current-home-script','./zuno-current-home.js','ZunoPlay: Home atual indisponível');injectStyle('zunoplay-current-home-mobile-style','./zuno-current-home-mobile.css');injectStyle('zunoplay-current-home-stats-style','./zuno-current-home-stats.css');injectStyle('zunoplay-current-interactions-style','./zuno-current-interactions.css');injectStyle('zunoplay-home-reference-v152-style','./zuno-home-reference-v152.css');injectStyle('zunoplay-current-viewport-style','./zuno-current-viewport.css');injectStyle('zunoplay-current-viewport-v154-style','./zuno-current-viewport-v154.css');injectStyle('zunoplay-current-hero-separation-style','./zuno-current-hero-separation.css');injectStyle('zunoplay-current-home-polish-v164-style','./zuno-current-home-polish-v164.css');
    }
    if(page==='avatar.html'){injectStyle('zunoplay-current-avatar-studio-style','./zuno-current-avatar-studio.css');injectScript('zunoplay-avatar-studio-reference-v167','./zuno-avatar-studio-reference-v167.js','ZunoPlay: layout do Avatar Studio indisponível')}
    if(page==='zuno-stack.html'){injectStyle('zunoplay-stack-premium-pieces-style','./zuno-stack-pieces.css');injectScript('zunoplay-stack-premium-pieces-script','./zuno-stack-pieces.js','ZunoPlay: peças premium do Zuno Stack indisponíveis')}
  }
  function mount(){applyBodyIdentity();if(page==='index.html')decorateHome();if(page==='sala.html')decorateRoom();if(page==='avatar.html')decorateAvatar();if(page==='comunidades.html')decorateCommunities();if(page==='jogos.html')decorateGames();installV167Refresh();loadCurrentModules()}
