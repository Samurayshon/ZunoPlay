...
function syncRelay(){const sig=sigRelay();if(!lastRelay){lastRelay=sig;return}if(sig===lastRelay)return;const prev=lastRelay.split('|').filter(x=>x&&x!=='-').length,next=sig.split('|').filter(x=>x&&x!=='-').length,now=Date.now();if(next>prev&&now-lastSendAt<1000){relaySends++;pulse('PEÇA ENVIADA · +20','send')}else if(next<prev&&now-lastTakeAt<1000){relayUses++;pulse('PEÇA RECEBIDA · +10','take')}lastRelay=sig}
...
function boot(){lastRelay=sigRelay();lastTray=sigTray();lastMatches=matches();removeBadge();sync();new MutationObserver(schedule).observe(document.querySelector('.shell')||document.body,{subtree:true,childList:true,attributes:true,attributeFilter:['class']});document.addEventListener('visibilitychange',()=>{if(!document.hidden)sync()})}
