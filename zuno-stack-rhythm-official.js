...
const R={phase:'',lastEvent:'',lastEventAt:0,started:false};
const PHASES=[
...
function fireEvent(key){const now=Date.now();if(!key||key===R.lastEvent&&now-R.lastEventAt<18000)return;R.lastEvent=key;R.lastEventAt=now;document.body.dataset.zsrEvent=key;const map={
 'final-minute':['ÚLTIMO MINUTO','Cada escolha pesa agora','critical'],
 'tray-danger':['BANDEJA NO LIMITE','Priorize trios antes de empilhar mais','warning'],
 'critical-wave':['ONDA CRÍTICA','A arena entrou em máxima tensão','critical'],
 'clean-flow':['FLUXO LIMPO','Espaço de sobra — acelere o combo','positive']
};const m=map[key];if(m)banner(m[0],m[1],m[2]);clearTimeout(fireEvent._t);fireEvent._t=setTimeout(()=>{if(document.body.dataset.zsrEvent===key)delete document.body.dataset.zsrEvent},5200)}
function sync(){mount();const playing=document.body.classList.contains('zstack-playing');if(!playing){R.started=false;R.phase='';delete document.body.dataset.zsrPhase;delete document.body.dataset.zsrEvent;return}const sec=seconds(),t=tray(),p=phaseFor(sec);if(!R.started){R.started=true;R.lastEvent='';R.lastEventAt=0}setPhase(p);fireEvent(eventKey(sec,t,p.id));const root=$('#zsrRhythm');if(root){root.dataset.phase=p.id;root.dataset.pressure=pressure().toLowerCase();root.title=`${p.label} · ${sec}s restantes`}}
function boot(){mount();setInterval(sync,500);document.addEventListener('visibilitychange',()=>{if(!document.hidden)sync()});sync()}
