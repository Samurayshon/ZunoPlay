(()=>{
  ...
  async function leaveEveryRoom(){if(!sb||!user)return;const{error}=await sb.from('room_members').delete().eq('user_id',user.id);if(error)console.warn('ZunoPlay: não foi possível limpar participação anterior.',error);else{sessionStorage.removeItem('zunoplay_room_id');sessionStorage.removeItem('zuno_return_room_id');sessionStorage.removeItem('zuno_return_room_url')}}
  async function touchCurrentRoom(){if(stopped||!sb||!user||!roomId)return false;const{data,error}=await sb.rpc('touch_room_session',{p_room_id:roomId});if(error){console.warn('ZunoPlay: heartbeat da sala falhou.',error);return false}if(data!==true){stopped=true;if(heartbeat)clearInterval(heartbeat);sessionStorage.removeItem('zunoplay_room_id');sessionStorage.removeItem('zuno_return_room_id');window.dispatchEvent(new CustomEvent('zuno:room-membership-ended',{detail:{room_id:roomId}}));return false}sessionStorage.setItem('zunoplay_room_id',roomId);if(preserveRoom){sessionStorage.setItem('zuno_return_room_id',roomId);sessionStorage.setItem('zuno_return_room_url','sala.html?room='+encodeURIComponent(roomId))}return true}
  async function startRoomHeartbeat(){if(!roomId||!sb||!user)return;for(let i=0;i<20&&!stopped;i++){const ok=await touchCurrentRoom();if(ok)break;await sleep(250)}if(stopped)return;heartbeat=setInterval(()=>touchCurrentRoom().catch(()=>{}),15000)}
  async function start(){sb=await waitClient();if(!sb){if(!PUBLIC_PAGES.has(page))redirectToLogin();return}user=await validateUser();if(!user){sessionStorage.removeItem('zunoplay_room_id');if(!PUBLIC_PAGES.has(page))redirectToLogin();return}window.dispatchEvent(new CustomEvent('zuno:auth-validated',{detail:{user_id:user.id}}));if(page==='sala.html'||preserveRoom){await startRoomHeartbeat();return}await leaveEveryRoom()}
