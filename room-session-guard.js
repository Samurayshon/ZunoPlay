(()=>{
  ...
  const ROOM_GAME_PAGES=new Set(['zuno-core.html','zuno-stack.html']);
  ...
  function clearReturnContext(){
    if(page==='sala.html'||preserveRoom)return;
    sessionStorage.removeItem('zuno_return_room_id');
    sessionStorage.removeItem('zuno_return_room_url');
  }
  ...
  async function validateCurrentMembership(){
    if(page!=='sala.html'||!roomId||!sb||!user)return true;
    const{data,error}=await sb.from('room_members').select('room_id,user_id,seat_index').eq('room_id',roomId).eq('user_id',user.id).maybeSingle();
    if(error){console.warn('ZunoPlay: não foi possível validar participação da sala.',error);return false}
    if(data){sessionStorage.setItem('zunoplay_room_id',roomId);return true}
    clearRoomContext();
    window.dispatchEvent(new CustomEvent('zuno:room-membership-ended',{detail:{room_id:roomId,reason:'not_member'}}));
    location.replace('salas.html?room_ended=1');
    return false;
  }
  async function touchCurrentRoom(){
    ...
    if(data!==true){
      clearRoomContext();
      window.dispatchEvent(new CustomEvent('zuno:room-membership-ended',{detail:{room_id:roomId,reason:'removed'}}));
      if(page==='sala.html'||preserveRoom)location.replace('salas.html?room_removed=1');
      return false;
    }
  ...
  async function startRoomHeartbeat(){
    ...
    const touched=await touchCurrentRoom();
    if(!touched||stopped)return;
    heartbeat=setInterval(()=>touchCurrentRoom().catch(()=>{}),15000);
  }
  async function start(){
    sb=await waitClient();
    if(!sb){if(!PUBLIC_PAGES.has(page))redirectToLogin();return}
    user=await sessionUser();
    if(!user){clearRoomContext();if(!PUBLIC_PAGES.has(page))redirectToLogin();return}
    window.dispatchEvent(new CustomEvent('zuno:auth-validated',{detail:{user_id:user.id}}));
    if(page==='sala.html'){
      if(!await validateCurrentMembership())return;
      await startRoomHeartbeat();
      return;
    }
    if(preserveRoom){await startRoomHeartbeat();return}
    clearReturnContext();
  }
