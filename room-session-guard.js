(()=>{
  ...
  const ROOM_GAME_PAGES=new Set(['zuno-stack.html']);
  ...
  async function sessionUser(){if(!sb)return null;try{const{data:{session},error}=await sb.auth.getSession();if(error||!session?.user)return null;return session.user}catch(_){return null}}
